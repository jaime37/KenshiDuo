...
// the build defines KENSHICOOP_PROTOTEST, so SaveXfer.cpp's NetLink/engine-
// coupled parts are #ifdef'd out; coop::logLine/logErrLine are stubbed inertly
// in main.cpp. With g_testSaveRoot unpinned, saveFolderFor() falls back to
// %LOCALAPPDATA%\kenshi\save, which is what the LOCALAPPDATA redirect below
// drives (testSaveXferRoundTrip in main.cpp always unpins the root when done).
//
